...
vaccine_summ_zcta %>% 
  ...
  ggplot() +
  # rcartocolor::scale_fill_carto_c(type = "diverging", palette = "Earth", direction = 1,
  #                                 labels = scales::number_format(accuracy=1, scale = 1, big.mark = ",")) +
  # geom_sf(aes(fill=vax_administered_per_cap)) +
  # geom_sf(aes(fill=vax_administered_per_cap)) +
  geom_sf(data=capcog, fill = "#efefef", color = "#d9d9d9") +
  geom_sf(aes(fill=vax_administered_per_cap)) +
  geom_sf(data=travis_county_sf, fill = NA, color = "gray62") +
  geom_sf(data=arterials, color = "red", size = .3) +
  geom_sf(data=I35, color = "red", size = 1) +
  geom_sf_label(data = capcog, 
                aes(label = COUNTY), 
                size = 2.5, face = "bold", family = "LibreFranklin-Bold") +
  annotation_raster(img, 
                    xmin=-97.72, xmax=-97.66, 
                    ymin=30.525, ymax=30.58) +
  annotation_raster(img,
                    xmin=-97.88, xmax=-97.82, 
                    ymin=30.015, ymax=30.07) +
  coord_sf(crs = 4269, datum = NA, xlim = mapRange_travis_cnty[c(1:2)], ylim = mapRange_travis_cnty[c(3:4)]) +
  ...
  guides(fill = guide_colorbar(title = "Vaccine Doses Administered Per 1000 Persons, by Zip Code", title.position = "top",
                               barwidth = 40, ticks=TRUE, nbin = 10000, frame.colour = "black", frame.linewidth = 1, 
                               barheight = .9, direction="horizontal", title.hjust = .5), colour = FALSE) +
  labs(title = "Vaccine Doses Administered Per 1,000 Persons, by Zip Code",
       subtitle = "Zip Codes Shown: Travis County",
       x = NULL,
       y = NULL,
       caption = "File: 'COVID-19 Vaccine Administration Data by ZIP Code'\nSource: Texas Department of State Health Services\nChart By: @mrworthington") +
  ggsave("figures/doses_administered__per_cap_zcta.png", device="png", dpi=300, width = 12, height = 9)
