...
cdc_last_update <- "Mar 7, 2021" 
cdc_last_time <- "12:25PM"

cdc_vaccine_summary_ggplot2 <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-5.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
...
cdc_vaccine_summary <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-5.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
  select(area=state_territory_federal_entity, contains("_per_100k"), starts_with("percent"), -contains("18")) %>%
  drop_na(doses_delivered_per_100k)
