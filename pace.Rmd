...
vaccine_summ %>%
  ...
  annotate(geom = "label", x = fully_vac_xend*1.54, y = .45, color = "#2d2d2d", 
...
```{r daily-dose-chart, eval=FALSE, include=FALSE, layout="l-page"}

vaccine_summ_daily <- read_excel(filepath, sheet = 4, skip=0, col_types = c("text", 
        "date", "numeric")) %>% 
  clean_names() %>%
...
vaccine_summ_daily %>% 
      hchart("column", hcaes(x = date, y = daily_doses), 
             animation=FALSE,
             color = "#333f48") %>% 
      hc_add_series(vaccine_summ_daily, type = "area", hcaes(x = date, y = new_doses_7_day_avg),
                    tooltip = list(pointFormat = "<br>7-Day Avg.: {point.new_doses_7day_avg:,.0f}"),
      ...
      hc_title(
        ...
        useHTML = TRUE) %>% 
hc_subtitle(
        text ="LEGEND - <span style='color: #bf5700'>7-DAY ROLLING AVG.</span>",
        useHTML = TRUE) %>% 
...
cdc_last_update <- "Mar 25, 2021" 
cdc_last_time <- "1:47PM"

cdc_vaccine_summary <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-8.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
