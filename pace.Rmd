...
vaccine_summ %>%
  ...
  geom_text(data = vaccine_summ %>% filter(phase_pop_est == "Future Phases"), color="#2d2d2d", size = 6,  hjust = 0.35,
            family = "Graphik-Semibold", aes(y=county_name, x=lab_pos), label = "Phase 1C + Pop. (16+)") +
...
highchart(type = "stock") %>% 
      ...
      hc_title(
        ...
        useHTML = TRUE) %>%
hc_subtitle(
        text ="Shown are the number of reported daily administered doses across the state of Texas and the 7-day average of daily administered doses.<br>LEGEND - <span style='color: #bf5700'>7-DAY ROLLING AVG.</span>",
        useHTML = TRUE) %>% 
...
```{r ggplot2 national comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.width=11, fig.height=4.5, layout="l-page"}

cdc_last_update <- "Mar 31, 2021" 
cdc_last_time <- "12:39PM"

cdc_vaccine_summary <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-9.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
...
<!-- ## Ages 16-49 years {.unnumbered} -->

<!-- ```{r daily-dose-chart-16, echo=FALSE, layout="l-page"} -->

<!-- library(quantmod) -->

<!-- vaccine_summ_daily <- read_excel(filepath, sheet = 4, skip=0, col_types = c("text", "date", "numeric")) %>%  -->
<!--   clean_names() %>%  -->
<!--   filter(age_group=="16-49 years") %>%  -->
<!--   group_by(date=vaccination_date) %>% -->
<!--   summarise(daily_doses = sum(doses_administered)) %>%  -->
<!--   mutate(date=as.Date(date)) %>%  -->
<!--   mutate(new_doses_7_day_avg= rollmean(daily_doses, 7, fill =0, -->
...
<!--       hc_plotOptions(area = list(fillOpacity=.1)) %>%  -->
<!--       hc_title( -->
<!--         text ="Daily Administered Doses - 16 to 49 Years", -->
<!--         useHTML = TRUE) %>% -->
<!-- hc_subtitle( -->
...
1. **Daily Average**: This is an ongoing calculation of daily averages that starts on Dec 15. The only thing it considers is how many doses are going into arms each day. It's important to note doses in arms is not the same thing as new persons. Some folks are turning for second doses, so this metric only captures the daily average of doses administered.
1. **When Texas will begin reaching Herd Immunity territory**: Based on that daily average, this box considers the remaining doses needed across partially vaccinated Texans and yet-to-be-vaccinated Texans. For those who haven't received a dose, this box considers how much we can expect to cover of remaining Texans with the current portfolio of 2-dose vaccines and 1-dose vaccines. Assuming all of that, this box then counts the number of days remaining between today and the minimum threshold of what epidemiologists have identified as "herd immunity" for COVID-19. [There's debate about the exact percentage](https://www.nytimes.com/interactive/2021/02/20/us/us-herd-immunity-covid.html), but vaccinating 70% of the *entire* Texas population would be a significant mnilestone for Texas. This is not a herd immunity projection, rather a look ahead as to when most of the population aged 16 and older will likely be vaccinated. 
