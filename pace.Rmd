...
vaccine_summ %>%
  ...
  scale_fill_manual(values = c("#333f48","#005f86",  "#9cadb7","#bf5700")) +
  ...
  annotate(
    geom = "curve", color = "#2d2d2d", x = fully_vac_xend*2.25, y = .48, xend = fully_vac_xend*1.1, yend = .54 , 
    curvature = 0, arrow = arrow(length = unit(2, "mm"))
  ) +
  annotate(geom = "label", x = fully_vac_xend*2.3, y = .5, color = "#2d2d2d", 
...
```{r ggplot2 national comparison, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

cdc_last_update <- "Mar 18, 2021" 
cdc_last_time <- "3:11PM"

cdc_vaccine_summary_ggplot2 <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-6.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
...
cdc_vaccine_summary_ggplot2 <- cdc_vaccine_summary %>% 
  ...
  mutate(indicator = str_replace(indicator,"doses_delivered_per_100k","Delivered Per 100k"),
         indicator = str_replace(indicator,"doses_administered_per_100k_by_state_where_administered","Administered Per 100k"),
         indicator = str_replace(indicator,"percent_of_total_pop_with_1_doses_by_state_of_residence","With 1 Dose"),
         indicator = str_replace(indicator,"percent_of_total_pop_with_2_doses_by_state_of_residence","With 2 Doses")) %>% 
  filter(str_detect(indicator, "Delivered|Administered")) %>% 
  ungroup()

cdc_vaccine_summary_ggplot2 %>% 
  filter(indicator == "Delivered Per 100k") %>%
  ...
  geom_text(data = cdc_vaccine_summary_ggplot2 %>% filter(indicator == "Delivered Per 100k", area != "Texas"), 
  ...
  labs(title = "How Texas Vaccine Deliveries Stacks Up Against All US States", 
       subtitle = paste0("Date is from the CDC. Data reflects reporting as of ",cdc_last_update," at ", cdc_last_time),
       y = "Doses Delivered Per 100k Persons",
       x = "States, Territories, and other US Entities",
       caption = "Source: 'COVID Data Tracker - Vaccines', Center for Disease Control.") +
  ggsave("figures/tx_us_comparison_vaccine_deliveries.png", device = "png", dpi = 300, width =13, height = 7)

cdc_vaccine_summary_ggplot2 %>% 
  filter(indicator == "Administered Per 100k") %>%
  ggplot(aes(x=reorder(area, desc(value)), y=value, fill = value)) +
  geom_bar(stat = "identity") +
  geom_point(aes(x=reorder(area, desc(value)), y=value), shape = 21, fill = "#bf5700", color = "black", size = 10, stroke = .1) +
  geom_text(angle=90, color="#ffffff", size = 3.1, family = "LibreFranklin-SemiBold",
            aes(y= 1000, x=area, label = area), hjust = 0) + 
  gghighlight(area == "Texas") +
  geom_text(data = cdc_vaccine_summary_ggplot2 %>% filter(indicator == "Administered Per 100k", area != "Texas"), 
            angle=90, color="#2d2d2d", size = 3.1, family = "LibreFranklin-SemiBold", aes(y= 1000, x=area, label = area), hjust = 0) +
  geom_point(aes(x=reorder(area, desc(value)), y=value), shape = 21, fill = "#bf5700", color = "black", size = 10, stroke = .1) +
  geom_text(data = cdc_vaccine_summary_ggplot2 %>% filter(indicator == "Administered Per 100k", area == "Texas"), 
            angle=90, color="#ffffff", size = 3.5, family = "LibreFranklin-Bold", aes(y= value, x=area, label = rank)) +  
  ...
  theme(axis.text.x = element_blank(),
        ...
        axis.ticks.x = element_blank()) +
  scale_y_continuous(label = scales::comma) +
  labs(title = "How Texas Vaccine Administration Stacks Up Against All US States", 
       subtitle = paste0("Date is from the CDC. Data reflects reporting as of ",cdc_last_update," at ", cdc_last_time),
       y = "Doses Delivered Per 100k Persons",
       x = "States, Territories, and other US Entities",
       caption = "Source: 'COVID Data Tracker - Vaccines', Center for Disease Control.") +
  ggsave("figures/tx_us_comparison_vaccine_administration.png", device = "png", dpi = 300, width =13, height = 7)

```


```{r highcharter nat comparison, echo=FALSE, layout="l-page"}
...
cdc_vaccine_summary <- read_csv("raw_data/covid19_vaccinations_in_the_united_states-6.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
  select(area=state_territory_federal_entity, contains("_per_100k"), starts_with("percent"), -contains("18")) %>%
  drop_na(doses_delivered_per_100k)
...
#   group_by(indicator) %>% 
#   mutate(rank = dense_rank(desc(value))) %>% 
#   mutate(indicator = str_replace(indicator,"doses_delivered_per_100k","Distributed"),
#          indicator = str_replace(indicator,"doses_administered_per_100k_by_state_where_administered","Administered"),
#          indicator = str_replace(indicator,"percent_of_total_pop_with_1_doses_by_state_of_residence","With 1 Dose"),
#          indicator = str_replace(indicator,"percent_of_total_pop_with_2_doses_by_state_of_residence","With 2 Doses")) %>% 
#   filter(str_detect(indicator, "Distributed|Administered"))
# 
# cdc_vaccine_summary_ggplot2 %>% 
#   ggplot(aes(x=area, y=reorder(value, rank))) +
#   geom_col() +
...
cdc_vaccine_column <- cdc_vaccine_summary %>%
  ...
  mutate(indicator = str_replace(indicator,"doses_delivered_per_100k","Distributed"),
         ...
         indicator = str_replace(indicator,"percent_of_total_pop_with_1_doses_by_state_of_residence","With 1 Dose"),
         indicator = str_replace(indicator,"percent_of_total_pop_with_2_doses_by_state_of_residence","With 2 Doses")) %>% 
  filter(indicator != "doses_Administered_by_state_where_administered",
         indicator != "doses_Administered_by_state_where_administered",
         !str_detect(indicator, "Dose"))
...
cdc_vaccine_drilldown <- cdc_vaccine_summary %>%
  ...
  filter(!str_detect(indicator, "Dose"))
...
```{r message=FALSE, warning=FALSE, include=FALSE, layout="l-screen-inset"}
...
<details class = "l-body">
<summary><strong>Question  | </strong>Why do they only measure the population aged 16 and above?</summary>
