...
title: "Explore Pace"
description: |
  How fast are we distributing COVID-19 Vaccines?
output: 
...
* Clinical staff providing diagnostic, laboratory, and/or rehabilitation services to Non 9-1-1 transport for routine care
...
* Last responders who provide mortuary or death services to decedents with COVID-19. Includes:
    - Embalmers and funeral home workers who have direct contact with decedents to Medical examiners and other medical certifiers who have direct contact with decedents
...
The chart attempts to capture a holistic snapshot of multiple data points related to state's current efforts to vaccinate the population. At a thirty thousand foot level, this chart is a single box that represents everyone in Texas aged 16 years and older. Within the box, you can see how much of is made up of eligible phases and how much of it is made up of the remaining population aged 16 years and older. And then, two lines mark the following:
...
cdc_last_update <- "Feb 1, 2021" 
cdc_last_time <- "1:55PM"
...
cdc_vaccine_summary <- read_csv("raw_data/covid19_vaccinations_in_the_united_states.csv",
                                skip = 2, na = "N/A") %>% 
  clean_names() %>% 
  select(area=state_territory_federal_entity, contains("_per_100k")) %>%
  mutate_at(vars(contains("_per_100k")), as.numeric) %>% 
  drop_na(distributed_per_100k)
...
cdc_vaccine_column <- cdc_vaccine_summary %>%
  ...
  mutate(indicator = str_replace(indicator,"distributed_per_100k","Distributed"),
...
vaccine_cnty_summ <- read_excel(filepath, sheet = "By County", skip=0) %>% 
  ...
  select(county_name, population_16, total_doses_allocated, doses_allocated_per_cap, people_fully_vaccinated, fully_vaccinated_per_cap) %>%   arrange(desc(doses_allocated_per_cap))
...
reactable(data =vaccine_cnty_summ,
          ...
          columns = list(
            ...
            people_fully_vaccinated = colDef(show = TRUE,  format = colFormat(separators = TRUE),
                                             align = "center", width = 250,
                                             header = with_tooltip("Full Vaccinations", "Number of Persons")),
            fully_vaccinated_per_cap = colDef(show = TRUE,
                                              header = with_tooltip("Full Vaccinations (Per Capita)", "Miles per US gallon for every 100,000 persons"),
                                             style = list(borderRight = "1.5px solid rgba(0, 0, 0, 0.2)", fontFamily = "Fira Code", whiteSpace = "pre"),
                                              align = "left",
                                              cell = function(value) {
