...
```{r inaccessible_pop, echo=FALSE, fig.height=11, fig.width=11, layout="l-body"}
...
inaccessible_vaccine_hub <-read_rds("clean_data/inaccessible_vaccine_hub.rds")
...
#               popup = ~GEOID)# %>% 
  ...
  ggplot() +
  geom_sf(data = inaccessible_vaccine_hub, fill="#5da5da", color = NA, show.legend = TRUE) +
  # geom_sf(data = driving_combined_30, fill="#bf5700", color = NA) +
  # geom_sf(data = texas_pop_block_grps, fill=NA, color = "black", lwd=.2) + 
  theme_minimal(base_family = "Graphik") +
  coord_sf(datum = NA) +
  theme(plot.title = element_text(family = "Graphik-Bold", color = "#333f48", size =20),
        plot.subtitle = element_text(family = "Graphik-Regular", color = "#333f48", margin = margin(b=15)),
        ...
        axis.text.x = element_blank(),
        legend.position = "none") +
  labs(title = "~7.4 million Texans live more than 30min\naway from a Vaccine Hub",
       subtitle = paste0("Source: Texas Department of State Health Services. As of ", today_lbl, " at ", time_lbl),
       x = NULL,
       y = NULL,
       color = NULL,
       caption = "Data File: 'Texas Vaccine Provider Data' | U.S. Census Bureau, ACS 5-Year Series \nLink: https://genesis.soc.texas.gov/files/accessibility/vaccineprovideraccessibilitydata.csv\nChart by: @mrworthington") #+
