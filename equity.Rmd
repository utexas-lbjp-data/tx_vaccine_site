...
```{r covid_case_fatality_dem_chart, eval=FALSE, include=FALSE}
...
dem_tx_data <- get_acs(geography = "state",
  ...
  select(dem_group = variable, pop_value = estimate, pop_pct = pct)
...
dem_data_full <- bind_rows(dem_cf_data,
  ...
  mutate(measure_f = factor(measure, levels=c('Doses Administered','Partially Vaccinated','Fully Vaccinated','Cases', 'Fatalities', 'State Population'))) %>% 
  ...
  mutate(pct_difference = pct-pop_pct)

  ggplot() +
  geom_col(data=dem_data_full %>% filter(measure!="State Population", dem_group!="Unknown"),
           aes(y=reorder(dem_group, pct_difference), x=pct_difference, fill=dem_group), stat="identity", na.rm=TRUE, width=.9) +
  facet_wrap(~measure, ncol=1) +
  # geom_vline(data=dem_data_full %>% filter(measure=="State Population", dem_group!="Unknown"), aes(xintercept=pct)) +
  # geom_bar(data=dem_data_full %>% filter(measure!="State Population", dem_group!="Unknown"),
  #          aes(y=reorder(dem_group, pct), x=pct, fill=dem_group), stat="identity", na.rm=TRUE, width=.9) +
  # geom_bar(data=dem_data_full %>% filter(measure=="State Population", dem_group!="Unknown") %>% select(-measure,-measure_f), 
  #          aes(y=reorder(dem_group, pct), x=pct), fill="#ffffff", color = NA, 
  #          alpha=.7,stat="identity", na.rm=TRUE, width=.9, show.legend=FALSE) +
  # geom_bar(data=dem_data_full %>% filter(measure=="State Population", dem_group!="Unknown") %>% select(-measure,-measure_f), 
  #          aes(y=reorder(dem_group, pct), x=pct), fill=NA, color = "#3d3d3d", 
  #          lwd = .5, linetype = 2, stat="identity", na.rm=TRUE, width=.9,, show.legend = FALSE) +
  hrbrthemes::scale_x_percent(limits = c(-.5, .5)) +
  ...
  theme(plot.title = element_text(family = "Graphik-Bold", color = "#333f48", size =20),
        ...
        plot.caption = element_markdown(lineheight = 1, hjust=0),
        axis.text.x = element_text(angle = 90),
        legend.title = element_markdown(), 
        legend.position = "bottom") +
  labs(title = "Comparing Cases, Fatalities, & Vaccines to Population Trends",
       subtitle = paste0("This chart shows break downs of known COVID Cases, COVID Fatalities, and Vaccine Data for Texas. Excluded are the overall\nrace and ethnic groups classified as 'Unknown' by DSHS. Administered doses is a summary figure of persons who have\nreceived a single dose and persons who are fully vaccinated.\nData pulled from sources on ", today_lbl, " at ", time_lbl),
       x = NULL,
       y = NULL,
       color = NULL,
       # fill = "Legend",
  ...
  # ggsave(paste0(today,"_","tx_covid_equity_analysis.png"), dpi = 300, width = 10, height = 8)
...
# ) -> xdf
# 
# dem_waffle <- dem_data_full %>% 
#   filter(measure!="State Population", 
#          dem_group!="Unknown",
#          dem_group!="Other") %>% 
#   mutate(waffle_pct = round(pct*100, digits =0))
# 
# install_fa_fonts()
# 
# dem_waffle %>% 
#   ggplot(aes(label = dem_group, values = waffle_pct)) +
#   facet_wrap(~measure_f, ncol=1) +
#   geom_pictogram(
#     n_rows = 20, size = 6, aes(colour = dem_group), flip = TRUE,
#     family = "FontAwesome5Brands-Regular"
#   ) +
#   scale_color_manual(
#     name = NULL,
#     values = c("#073f9c", "black", "#f34323", "#bf5700"),
#     labels = c("BitBucket", "GitHub", "Other", "Home")
#   ) +
#   scale_label_pictogram(
#     name = NULL,
#     values = c("bitbucket", "github", "git-alt", "apple"),
#     labels = c("BitBucket", "GitHub", "Other", "Home")
#   ) +
#   coord_equal() +
#   # theme_ipsum_rc(grid="") +
...
# 
# ggplot(data=dem_waffle, aes(fill=dem_group, values=waffle_pct)) +  
#   geom_waffle(color = "white", size=1, flip=TRUE) +
...
```{r accessible_pop,  echo=FALSE, layout = "l-page"}
...
time_pulled <-  unique(vaccine_sites_sf$time_pulled)
  ...
  theme(plot.title = element_text(family = "Graphik-Bold", color = "#333f48", size =24),
        ...
        plot.margin = margin(t=15, r=15, b=10, l = 15)) +
  labs(title = "~5.7m Texans are a 30+min drive from a Vaccine Hub",
       subtitle = paste0("Shown below are census blocks where the population aged 16 years and older would<br> have to drive 30 minutes or more to one of the state's existing Vaccine Hubs. Areas<br>marked in orange are within a 30 min drive to a Vaccine Hub.</span><br>Hub Locations Accurate as of ", date_pulled, " at ", time_pulled),
       fill = "Legend",
      ...
      # ggsave(paste0(today,"_","inaccessible_hub_30min.png"), dpi = 300, width = 11, height = 11)
